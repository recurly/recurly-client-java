...
- Attributes passed in as part of the request will override either of the
  ...
  public List<SubscriptionAddOnUpdate> getAddOns() { return this.addOns; }

  
  /**
   * @param addOns If you provide a value for this field it will replace any
existing add-ons. So, when adding or modifying an add-on, you need to
include the existing subscription add-ons. Unchanged add-ons can be included
just using the subscription add-on''s ID: `{"id": "abc123"}`. If this
value is omitted your existing add-ons will be unaffected. To remove all
...
- Attributes passed in as part of the request will override either of the
  ...
  public void setAddOns(final List<SubscriptionAddOnUpdate> addOns) { this.addOns = addOns; }

  
  public SubscriptionChangeBillingInfoCreate getBillingInfo() { return this.billingInfo; }
  ...
  public void setBillingInfo(final SubscriptionChangeBillingInfoCreate billingInfo) { this.billingInfo = billingInfo; }
  ...
  /**
   * @param couponCodes A list of coupon_codes to be redeemed on the subscription during the change. Only allowed if timeframe is now and you change something about the subscription that creates an invoice.
   */
  ...
  /**
   * @param planCode If you want to change to a new plan, you can provide the plan's code or id. If both are provided the `plan_id` will be used.
   */
  ...
  /**
   * @param planId If you want to change to a new plan, you can provide the plan's code or id. If both are provided the `plan_id` will be used.
   */
  ...
  public SubscriptionChangeShippingCreate getShipping() { return this.shipping; }

  
  /**
   * @param shipping Shipping addresses are tied to a customer's account. Each account can have up to 20 different shipping addresses, and if you have enabled multiple subscriptions per account, you can associate different shipping addresses to each subscription.
   */
  
  public void setShipping(final SubscriptionChangeShippingCreate shipping) { this.shipping = shipping; }
  ...
  public Constants.ChangeTimeframe getTimeframe() { return this.timeframe; }

  
  /**
   * @param timeframe The timeframe parameter controls when the upgrade or downgrade takes place. The subscription change can occur now, when the subscription is next billed, or when the subscription term ends. Generally, if you're performing an upgrade, you will want the change to occur immediately (now). If you're performing a downgrade, you should set the timeframe to `term_end` or `bill_date` so the change takes effect at a scheduled billing date. The `renewal` timeframe option is accepted as an alias for `term_end`.
   */
  
  public void setTimeframe(final Constants.ChangeTimeframe timeframe) { this.timeframe = timeframe; }
  ...
  /**
   * @param unitAmount Optionally, sets custom pricing for the subscription, overriding the plan's default unit amount. The subscription's current currency will be used.
   */
